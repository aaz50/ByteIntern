...
1. **Create a GitHub repository and push your code**
   ...
   git remote add origin https://github.com/yourusername/ByteIntern.git
     ...
     check-jobs:
       ...
       steps:
         ...
         - name: Run job tracker
           ...
           run: python -m src.runners.local
...
3. **Add cron job** (runs every 6 hours)
   ```
   0 */6 * * * cd /full/path/to/ByteIntern && /usr/bin/python3 -m src.runners.local >> tracker.log 2>&1
   ```
...
4. Action: Start a program
   - Program: `python`
   - Arguments: `-m src.runners.local`
   - Start in: `C:\path\to\ByteIntern`
...
5. Add a start command: `while true; do python -m src.runners.local; sleep 21600; done`

### Heroku Setup (using scheduler add-on):

1. Create Heroku app
2. Add Heroku Scheduler add-on
3. Configure job to run `python -m src.runners.local` every hour/day
